...
<SoftTap>:
    ...
    FloatLayout:                            #in decending order as seen in GUI
        ...
        Button:
            ...
            on_press:
                root.manager.get_screen('soft_tap').duration.flag = '1'
        ...
        Button:
            ...
            flag: '0'
            on_press:
                if(root.manager.get_screen('soft_tap').start.flag == '0'):root.manager.get_screen('soft_tap').start.flag = '1'; root.manager.get_screen('soft_tap').start.text = 'Experement Underway'
                elif(root.manager.get_screen('soft_tap').start.flag == '1'):root.manager.get_screen('soft_tap').start.flag = '0';root.manager.get_screen('soft_tap').start.text = 'Done / Start'
        ...
        GridLayout:
            ...
            CustButton:
                ...
                on_press:
                    ...
                    elif((root.manager.get_screen('soft_tap').duration.flag)=='1'):root.manager.get_screen('soft_tap').duration.text = entry.text + ' Minute Experement'; root.manager.get_screen('soft_tap').duration.choice = entry.text                                                      
...
<MenuScreen>:
    ...
    FloatLayout:                            #in decending order as seen in GUI
        ...
        Button:
            ...
            flag: '0'
            on_press:
                if(root.manager.get_screen('menu').label5.flag == '0'):root.manager.get_screen('menu').label5.flag = '1'; root.manager.get_screen('menu').label5.text = 'Experement Underway'
...
class MenuScreen(Screen):
    ...
    def global_update(self):
        ...
        exp_time = int(sm.get_screen('soft_tap').duration.choice) 
    ...
    def start_stop(self):  # periodicaly                       (DO I NEED THIS )
        ...
        # hard drops start was activated recursive call till completion
        ...
        if (sm.get_screen('soft_tap').start.flag == '1'):
            sm.get_screen('soft_tap').start.flag = '0'  # clear flag
 
            # schedualing the end of soft tap mod base on experement time
            MenuScreen.global_update(self)
            Clock.schedule_once(Events.soft_mode_end, exp_time)
            Events.toggle_hold_out(self)
            Events.soft_mode(self)
        elif(sm.get_screen('soft_tap').start.flag == '0'):
            pass
...
class Events(Screen):
    ...
    def manage_drops(self):  # CONTROL
        ...
        MenuScreen.global_update(self)        #                       (NO )-> schedual disengage,  engage->lift (idel)
        global drops_left
        global first_drop
        global main_interval
        global start
    ...
    def soft_mode(self):
        global exp_time
        global soft_tap_done
        print(exp_time)
        if(soft_tap_done == False):
            print('lift')
            for i in range(1000):
                kit1.stepper1.onestep(direction=stepper.FORWARD, style=stepper.DOUBLE)     #lift
            kit1.stepper1.release()  #so motor current dosnt spike
            print('lower')
            for i in range(1000): #i put this a lil higher then lift to ensure absolute zero because bump sensor didnt come in 
                kit1.stepper1.onestep(direction=stepper.BACKWARD, style=stepper.DOUBLE)     #lower
            kit1.stepper1.release()   
            
            Clock.schedule_once(Events.soft_mode, .1)  # recursive call
        elif(soft_tap_done == True):
            print('exiting soft tap mode')
            soft_tap_done = False
            pass
 
    def soft_mode_end(self):
        global soft_tap_done
