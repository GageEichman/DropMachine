...
<HomeScreen>
    ...
    FloatLayout:
        ...
        Button:
            ...
            text: '30 second recording'
            font_size: 40
            size_hint : (.15,.17)
            pos_hint : {'top':.35,'center_x':.15}
            
            on_press:
                root.manager.get_screen('home').camtime.flag = '1' 
        ...
        GridLayout:
            ...
            CustButton:
                ...
                on_press:
                    ...
                    elif((root.manager.get_screen('drop_screen').label2.flag) == '1'):root.manager.get_screen('drop_screen').label2.text = entry.text + ' minute intervals' ; root.manager.get_screen('drop_screen').label2.choice = entry.text
                    ...
                    elif((root.manager.get_screen('tap_screen').interval.flag)=='1'):root.manager.get_screen('tap_screen').interval.text = entry.text + ' Minute Intervals'; root.manager.get_screen('tap_screen').interval.choice = entry.text
                    ...
                    #video length button
                    elif((root.manager.get_screen('home').camtime.flag)=='1'):root.manager.get_screen('home').camtime.text = entry.text + ' second recording'; root.manager.get_screen('home').camtime.choice = entry.text
                    ...
                    elif((root.manager.get_screen('home').camtime.flag)=='1'): root.manager.current = 'home'
                    ...
                    root.manager.get_screen('home').camtime.flag = '0'
...
class DropScreen(Screen):
    ...
    def Tap_global_update(self):
        ...
        Tap_Interval = int(sm.get_screen('tap_screen').interval.choice) * 60 #in seconds
        Tap_Force = int(sm.get_screen('tap_screen').force.choice)
        Tap_Ammount = int(sm.get_screen('tap_screen').ammount.choice)
        video_length = int(sm.get_screen('home').camtime.choice)
    ...
    def Drop_global_update(self):
        ...
        main_interval = int(sm.get_screen('drop_screen').label2.choice) * 60 # convert to seconds
...
class Events(Screen):
    ...
    def Walk_to_threshold(self):
        ...
        while True:
            ...
            Sensor_Force_Value = random.randint(0,10000)
            List_Of_Values.append(Sensor_Force_Value)
            Newton_value = round((max(List_Of_Values)*(m_conversion)+(b_conversion)),2)
            #highest sensor value in print train will say newton force, easier this way
    ...
    def focus_camera(self):
        ...
        focusing(max_index)
        time.sleep(1)    
    #records until cam_stop_and_convert is called
    def cam_record(self):
        global drops_left
        global Taps_Left
        global start
    ...
    def Read_Sensor(self):
        ...
        while True:
            #All_Data = bus.read_i2c_block_data(Sensor_Address,0x00,6)
            #Sensor_Force_Value = (All_Data[4] << 8 | All_Data[5]) - 255
            Sensor_Force_Value = random.randint(0,1000)
            List_Of_Values.append(Sensor_Force_Value)
            TimeArray.append(Time_Running)
    ...
    def lower(self):  # LOWER
        ...
        print ('lowered, reversing slightly')
        '''
        while True:
            if bump.is_pressed:
                print('stopping')
